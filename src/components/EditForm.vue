<template>
	<div class="container">
		...
		<p class="lead">Para editar un registro debes buscar el codigo verificador y presionar el boton editar</p>
		<form @submit="save($event)">
			...
			<div v-if="found">
				<div class="form-row">
					...
					<div class="form-group col-6 col-md-6 col-lg-6">
						<label>Año</label>
						<input type="number" class="form-control" v-model="cols.anyo">
					</div>
					<div class="form-group col-6 col-md-6 col-lg-6">
						<label>Mes</label>
						<input type="number" class="form-control" v-model="cols.mes">
					</div>
					<div class="form-group col-6 col-md-6 col-lg-6">
						<label>Día</label>
						<input type="number" class="form-control" v-model="cols.dia">
					</div>
				</div>
				<hr>
				...
				<div class="form-row">
					...
					<div class="form-group col-12 col-md-6 col-lg-3">
						...
						<input type="number" step=".01" class="form-control" v-model="cols.intervencionOjoI">
					</div>
					<div class="form-group col-12 col-md-6 col-lg-3">
						<label>Intervención ojo derecho</label>
						<input type="number" step=".01" class="form-control" v-model="cols.intervencionOjoD">
					</div>
					<div class="form-group col-12 col-md-6 col-lg-3">
				...
				<div class="form-row">
					...
					<div class="form-group col-12 col-md-6 col-lg-3">
						...
						<input type="text" class="form-control" v-model="cols.complicacione">
					</div>
					<div class="form-group col-12 col-md-6 col-lg-3">
						<label>Insumos</label>
	...
	methods:{
		...
		async find(){
            ...
            if(res.data.length!=1){
				...
				this.cols.codigoVerificador=(editRow.codigoVerificador)?editRow.codigoVerificador:""
				this.cols.claveMexpharm=(editRow.claveMexpharm)?editRow.claveMexpharm:""
				this.cols.claveInst=(editRow.claveInst)?editRow.claveInst:""
				this.cols.sedeCamp=(editRow.sedeCamp)?editRow.sedeCamp:""
				this.cols.anyo=(editRow.anyo)?editRow.anyo:""
				this.cols.mes=(editRow.mes)?editRow.mes:""
				this.cols.dia=(editRow.dia)?editRow.dia:""
				this.cols.medicoResponsable=(editRow.medicoResponsable)?editRow.medicoResponsable:""
				this.cols.medicoTratante=(editRow.medicoTratante)?editRow.medicoTratante:""
				this.cols.date=(editRow.date)?editRow.date:""
				this.cols.nombre=(editRow.nombre)?editRow.nombre:""
				this.cols.sexo=(editRow.sexo)?editRow.sexo:""
				this.cols.edad=(editRow.edad)?editRow.edad:""
				this.cols.rEdad=(editRow.rEdad)?editRow.rEdad:""
				this.cols.edoCivil=(editRow.edoCivil)?editRow.edoCivil:""
				this.cols.otroEdoCivil=(editRow.otroEdoCivil)?editRow.otroEdoCivil:""
				this.cols.viveCon=(editRow.viveCon)?editRow.viveCon:""
				this.cols.otroViveCon=(editRow.otroViveCon)?editRow.otroViveCon:""
				this.cols.viveCuantos=(editRow.viveCuantos)?editRow.viveCuantos:""
				this.cols.viviendaEs=(editRow.viviendaEs)?editRow.viviendaEs:""
				this.cols.ingresoMensual=(editRow.ingresoMensual)?editRow.ingresoMensual:""
				this.cols.nPersonasAportan=(editRow.nPersonasAportan)?editRow.nPersonasAportan:""
				this.cols.localidad=(editRow.localidad)?editRow.localidad:""
				this.combos.entidad=(editRow.eFederativa)?findIndexInObjArr(this.combos.entidades,'estado',editRow.eFederativa):-1
				this.selectEntidad()
				this.combos.municipio=(this.combos.municipios)?findIndexInObjArr(this.combos.municipios,'municipio',editRow.municipio):-1
				this.selectMunicipio()
				this.cols.claveEFederativa=(editRow.claveEFederativa)?editRow.claveEFederativa:""
				this.cols.eFederativa=(editRow.eFederativa)?editRow.eFederativa:""
				this.cols.lugarNacimiento=(editRow.lugarNacimiento)?editRow.lugarNacimiento:""
				this.cols.ocupacion=(editRow.ocupacion)?editRow.ocupacion:""
				this.cols.actRemunerada=(editRow.actRemunerada)?editRow.actRemunerada:""
				this.cols.actRemuneradaCual=(editRow.actRemuneradaCual)?editRow.actRemuneradaCual:""
				this.cols.telefono=(editRow.telefono)?editRow.telefono:""
				this.cols.clacificacionOjo=(editRow.clacificacionOjo)?editRow.clacificacionOjo:""
				this.cols.intervencionOjoD=(editRow.intervencionOjoD)?editRow.intervencionOjoD:""
				this.cols.intervencionOjoI=(editRow.intervencionOjoI)?editRow.intervencionOjoI:""
				this.cols.procedimiento=(editRow.procedimiento)?editRow.procedimiento:""
				this.cols.observaciones=(editRow.observaciones)?editRow.observaciones:""
				this.cols.complicacione=(editRow.complicacione)?editRow.complicacione:""
				this.cols.insumos=(editRow.insumos)?editRow.insumos:""
				this.cols.folioConsignia=(editRow.folioConsignia)?editRow.folioConsignia:""
				this.combos.checkNoAct=(editRow.actAntesDeCirugia)?editRow.actAntesDeCirugia.split(','):[]
				this.cols.actAntesDeCirugia=(editRow.actAntesDeCirugia)?editRow.actAntesDeCirugia:""
				this.cols.nActAntesDeCirugia=(editRow.nActAntesDeCirugia)?editRow.nActAntesDeCirugia:""
				this.combos.checkAct=(editRow.actDespuesDeCirugia)?editRow.actDespuesDeCirugia.split(','):[]
				this.cols.oActAntesDeCirugia=(editRow.oActAntesDeCirugia)?editRow.oActAntesDeCirugia:""
				this.cols.actDespuesDeCirugia=(editRow.actDespuesDeCirugia)?editRow.actDespuesDeCirugia:""
				this.cols.nActDespuesDeCirugia=(editRow.nActDespuesDeCirugia)?editRow.nActDespuesDeCirugia:""
				this.cols.oActDespuesDeCirugia=(editRow.oActDespuesDeCirugia)?editRow.oActDespuesDeCirugia:""
				this.cols.satisfaccion=(editRow.satisfaccion)?editRow.satisfaccion:""
            }
		},
		selectEntidad(){
			if(this.combos.entidad!=""||this.combos.entidad!=-1||this.combos.entidad!="-1"){
